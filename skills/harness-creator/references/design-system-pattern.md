...
- **Stitch MCP** — a *remote* HTTP server at `https://stitch.googleapis.com/mcp` ([setup](https://stitch.withgoogle.com/docs/mcp/setup)). Add it in Claude Code with an API key (generate one in Stitch Settings → API Keys):
  ```
